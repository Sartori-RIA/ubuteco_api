...
RSpec.describe Api::OrdersController, type: :request do
  ...
  describe '#GET /api/orders' do
    it 'should request all orders' do
      get api_orders_path, headers: auth_header(user)
  ...
  describe '#GET /api/orders/:id' do
    it 'should request order by id' do
      get api_order_path(orders.sample.id), headers: auth_header(user)
      expect(response).to have_http_status(200)
    end
  end
  ...
  describe '#GET /api/orders/search' do
    it 'should search orders' do
      get search_api_orders_path, params: { q: 'tralala' }, headers: auth_header(user)
      expect(response).to have_http_status(200)
    end
  end
  ...
  describe '#POST /api/orders' do
    it 'should create a order' do
      ...
      post api_orders_path, params: attributes.to_json, headers: auth_header(user)
  ...
  describe '#PUT /api/orders/:id' do
    ...
    it 'should update a table' do
      put api_order_path(order.id), params: order.to_json, headers: auth_header(user)
      expect(response).to have_http_status(200)
    end
  end
  ...
  describe '#DELETE /api/orders/:id' do
    it 'should delete order' do
      delete api_order_path(orders.sample.id), headers: auth_header(user)
