...
json.extract!(beer, :id, :name, :price_cents, :price_currency, :quantity_stock,
              :description, :abv, :ibu, :valid_until, :created_at, :updated_at)
...
if beer.image.attached?
  json.image do
    json.url url_for(beer.image)
    json.filename beer.image.filename.to_s
    json.content_type beer.image.content_type
  end
end
